...
import { readScopedStorage, SELECTED_WORKSPACE_STORAGE_KEY, writeScopedStorage } from "../config/runtime.js";
...
export async function loadWorkspaceBundle(user) {
  ...
  const activeMembers = await must(
    client
  ...
  const ownedWorkspaceRows = await must(
    client
      .from("workspaces")
      .select("id")
  ...
  if (!activeMembers.length) {
    return {
      ...
      workspace: null,
      workspaces: [],
      ownedWorkspaceCount: ownedWorkspaceRows.length,
      role: "",
      projects: [],
      lines: [],
      members: [],
  ...
  const workspaceIds = activeMembers.map((member) => member.workspace_id);
  const workspaces = await must(
    client
      .from("workspaces")
  ...
  const selectedWorkspaceId = readScopedStorage(SELECTED_WORKSPACE_STORAGE_KEY, "");
  const workspace = workspaces.find((item) => item.id === selectedWorkspaceId) || workspaces[0];
  if (workspace.id !== selectedWorkspaceId) {
    writeScopedStorage(SELECTED_WORKSPACE_STORAGE_KEY, workspace.id);
  }
  const activeMember = activeMembers.find((member) => member.workspace_id === workspace.id) || activeMembers[0];
...
export function setSelectedWorkspace(workspaceId) {
  writeScopedStorage(SELECTED_WORKSPACE_STORAGE_KEY, workspaceId);
}
