...
function renderNav(state) {
  ...
  return `
    <nav class="side-nav" id="mobile-navigation" aria-label="Main navigation">
      <div class="mobile-workspace-picker">
        ${renderWorkspaceSwitcher(state)}
      </div>
      <ul class="nav-list">
        ${items.map((item) => `
          <li>
...
function renderWorkspaceSwitcher(state) {
  if (!state.workspaces || state.workspaces.length < 2) return "";
  return `
    <label class="workspace-switcher">
      <span class="sr-only">Active company</span>
      <select data-action="switch-workspace" aria-label="Active company">
        ${state.workspaces.map((workspace) => `
          <option value="${workspace.id}" ${state.workspace?.id === workspace.id ? "selected" : ""}>${escapeHtml(workspace.name)}</option>
        `).join("")}
      </select>
    </label>
  `;
...
export function bindShellView(root, handlers) {
  ...
  root.querySelectorAll("[data-action='switch-workspace']").forEach((select) => {
    select.addEventListener("change", (event) => {
      handlers.switchWorkspace(event.target.value);
    });
  });
