...
import { startExtraCompanyCheckout } from "./api/billingApi.js";
...
function renderBlockingOverlay(state) {
  if (!state.blockingStatus) return "";
  return `
    <div class="blocking-overlay" role="status" aria-live="polite" aria-busy="true">
      <div class="blocking-panel">
        <span class="loader" aria-hidden="true"></span>
        <strong>${state.blockingStatus.title || "Working..."}</strong>
        <p>${state.blockingStatus.message || ""}</p>
      </div>
    </div>
  `;
}
...
function bindCurrentView(state, activeRoute) {
  ...
  if (activeRoute === ROUTES.company) {
    bindAdminCompanyView(root, {
      ...
      startExtraCompanyCheckout: async (payload) => {
        setBlockingStatus({
          title: "Opening checkout",
          message: "You are being redirected to complete the $9 USD company payment.",
        });
        try {
          const url = await startExtraCompanyCheckout(payload);
...
function render() {
  ...
  root.innerHTML = renderShell(state, activeContent(state, activeRoute)) + renderBlockingOverlay(state);
...
function consumeCheckoutReturn() {
  ...
  if (!checkout) return;
  if (checkout === "extra-company-success") {
    setActionStatus({
      type: "success",
      message: "Payment received. The new company will appear here after processing.",
    });
    refreshWorkspaceData();
  }
  if (checkout === "extra-company-cancel") {
    setActionStatus({ type: "warning", message: "Checkout was canceled. No company was added." });
  }
  params.delete("checkout");
  const clean = `${window.location.pathname}${params.toString() ? `?${params.toString()}` : ""}${window.location.hash || "#/admin/company"}`;
